...
require_relative "mock_notify_client"
require_relative "./env_token_store"

module Services
  def self.notify
    if ENV['NOTIFY_BASE_URL'] && !ENV['NOTIFY_BASE_URL'].to_s.empty?
      puts "Using Mock Notify client with Base URL: #{ENV['NOTIFY_BASE_URL']}"
      @notify_client ||= MockNotifyClient.new(ENV['NOTIFY_BASE_URL'])
    else
      # --- PROD LOGIC ---
